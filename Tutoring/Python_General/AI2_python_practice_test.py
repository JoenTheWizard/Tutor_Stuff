...
def question1(name):
    ...
    '''
    name_format = name.lower().title() #Fix name format
    split = name_format.split(',') #Split by comma
    final_string = f'{split[1].strip()}, {split[0].strip()}' #Set the correct string from the splitted array
...
def question2(file_name):
    ...
    '''
    list_r = [] #Initialize the list to append and return at the end of function
    with open(file_name, 'r') as f: #Read file name parsed from function argument
        for i in f.readlines(): #Read each line from the file
            name = i.split(',') #Split the reading current line by ','
            f_name = f'{name[1].strip().upper()}, {name[0].strip().lower()}' #Set the firstname to UPPERCASE and the last name to lowercase
            list_r.append(f_name) #Add/append the `f_name` variable (final string result) to the list
    
    return list_r #At the end of the reading loop return the final result of the list
...
def question3(list_of_numbers):
    ...
    '''
    #Hint: Iterate through the array and use the modulos ('%') operator to append to list of even numbers
    #Modulo gets the remaining value, so when using modulo for value '2' it needs to return value 0.
    #Example: 4 % 2 == 0 ('4' is even) or 3 % 2 == 1 ('3' is not even)

#You need to pass the unittest here
